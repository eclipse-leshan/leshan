...
public class ObserveSpecTest {
    ...
    public void serialize_cancel_param() {
        ObserveSpec spec = new ObserveSpec.Builder().cancel().build();
        Assert.assertEquals("cancel", spec.toQueryParams()[0]);
    }

    @Test
    public void serialize_step_param() {
        Float step = 12.1f;
        ObserveSpec spec = new ObserveSpec.Builder().step(step).build();
        Assert.assertEquals(String.format("st=%s", step), spec.toQueryParams()[0]);
    }

    @Test
    public void serialize_greaterThan_param() {
        Float threshold = 12.1f;
        ObserveSpec spec = new ObserveSpec.Builder().greaterThan(threshold).build();
        Assert.assertEquals(String.format("gt=%s", threshold), spec.toQueryParams()[0]);
    }

    @Test
    public void serialize_lessThan_param() {
        Float threshold = 12.1f;
        ObserveSpec spec = new ObserveSpec.Builder().lessThan(threshold).build();
        Assert.assertEquals(String.format("lt=%s", threshold), spec.toQueryParams()[0]);
    }

    @Test
    public void serialize_minPeriod_param() {
        int seconds = 30;
        ObserveSpec spec = new ObserveSpec.Builder().minPeriod(seconds).build();
        Assert.assertEquals(String.format("pmin=%s", seconds), spec.toQueryParams()[0]);
    }

    @Test
    public void serialize_maxPeriod_param() {
        int seconds = 60;
        ObserveSpec spec = new ObserveSpec.Builder().maxPeriod(seconds).build();
        Assert.assertEquals(String.format("pmax=%s", seconds), spec.toQueryParams()[0]);
    }
