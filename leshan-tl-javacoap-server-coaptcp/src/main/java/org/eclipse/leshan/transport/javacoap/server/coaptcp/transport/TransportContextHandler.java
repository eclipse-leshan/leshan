    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof SslHandshakeCompletionEvent) {
            if (((SslHandshakeCompletionEvent) evt).isSuccess()) {
                ...
                TransportContext oldTransportContext = ctx.channel().attr(TRANSPORT_CONTEXT_ATTR)
                        .setIfAbsent(transportContext);
                if (oldTransportContext != null) {
                    throw new IllegalStateException(
                            String.format("Can not create new endpoint context %s as %s already exists.",
                                    transportContext, oldTransportContext));
                }
